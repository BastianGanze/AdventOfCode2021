...
use crate::bingo_board::{parse_board_data, BingoBoard, CheckNumberResult};
...
fn main() {
    match std::fs::read_to_string("./src/04.txt") {
        Ok(text) => {
            ...
            part_1(numbers, &mut bingo_boards);
...
fn part_1(numbers: Vec<u32>, bingo_boards: &mut HashMap<u32, BingoBoard<5>>) {
    'outer: for number in numbers {
        for board_id in play_game(number, bingo_boards) {
            println!(
                "Solution is 1 {}",
                bingo_boards
                    .get(&board_id)
...
fn part_2(numbers: Vec<u32>, bingo_boards: &mut HashMap<u32, BingoBoard<5>>) {
    ...
    for number in numbers {
        for board_that_one in play_game(number, bingo_boards) {
            let won_board = bingo_boards.remove(&board_that_one).unwrap();
